...
local function getNearestDoor()
    ...
    for _, pos in ipairs(doorPositions) do
        ...
        if d < dMin then best, dMin = pos, d end
    end
    return best
end
local function goUp()
    ...
    if door then
        ...
        task.wait(1.3)
        if root then root.CFrame += Vector3.new(0,200,0) end
    end
end
local function dropDown()
    if root then root.CFrame -= Vector3.new(0,50,0) end
end
...
frame.Position    = UDim2.new(1, -10, 0, 60)    -- *** cao hơn (60px từ trên) ***
...
RunService.Heartbeat:Connect(function()
    stroke.Color = Color3.fromHSV((tick()*0.2)%1, 1, 1)  -- rainbow border
end)
...
toggle.Text, toggle.BackgroundColor3 = "Tuấn Anh IOS", Color3.fromRGB(200,60,60)
